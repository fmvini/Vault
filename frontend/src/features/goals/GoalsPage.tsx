...
export function GoalsPage() {
  ...
  const save = useMutation({
    ...
    onError: (cause) => setError(axios.isAxiosError<{ detail?: string }>(cause) ? cause.response?.data?.detail ?? 'Não foi possível salvar o limite.' : 'Não foi possível salvar o limite.')
  });
  const remove = useMutation({
    mutationFn: async (id: string) => api.delete(`/goals/${id}`),
    onSuccess: invalidate,
    onError: () => setError('Não foi possível excluir o limite.')
  });
  ...
  function removeGoal(goal: ApiGoal) { if (window.confirm(`Excluir o limite de ${goal.category_name}?`)) remove.mutate(goal.id); }

  return <div className='feature-page'>
    <PageHeader title='Limites do mês' description='Defina quanto pretende gastar em cada categoria neste mês.' action={<button className='primary-button compact' onClick={openCreate}><Plus size={17} />Novo limite</button>} />
    ...
    {goals.isError && <p className='form-error' role='alert'>Não foi possível carregar os limites.</p>}
    {goals.isFetching && <p className='empty-row' role='status'>Carregando limites...</p>}
    {remove.isPending && <p className='empty-row' role='status'>Excluindo limite...</p>}
    <div className='goal-overview'><div><Target /><span>{active.length} limites ativos<strong>{formatMoney(planned, user?.default_currency ?? 'BRL')} planejados</strong></span></div><p>Você utilizou {totalPercent}% dos limites definidos neste mês.</p></div>
    <section className='goal-board' aria-busy={goals.isLoading}>{active.map((goal) => { const percent = Math.round(Number(goal.current_month_spent) / Number(goal.monthly_limit) * 100); return <article key={goal.id} className={goal.is_exceeded ? 'exceeded' : ''}><header><span>{goal.category_name}</span><em>{percent}%</em></header><strong>{formatMoney(Number(goal.current_month_spent), goal.currency)} <small>de {formatMoney(Number(goal.monthly_limit), goal.currency)}</small></strong><div className='goal-track'><span style={{ width: `${Math.min(percent, 100)}%` }} /></div><footer><span>{goal.is_exceeded ? 'Limite ultrapassado' : 'Dentro do planejado'}</span><span className='row-actions'><button onClick={() => openEdit(goal)} aria-label={`Editar limite de ${goal.category_name}`}><Pencil size={14} /></button><button onClick={() => removeGoal(goal)} aria-label={`Excluir limite de ${goal.category_name}`}><Trash2 size={14} /></button></span></footer></article>; })}{!goals.isLoading && !active.length && <p className='empty-row'>Nenhum limite ativo. Crie um para acompanhar seus gastos.</p>}</section>
