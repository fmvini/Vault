...
- Após o push de `ad68e09`, a integração Git gerou deploys automáticos de produção: API `https://vault-6kcao1nyu-fmvini-projects.vercel.app` (Ready em 22 s) e frontend `https://vault-8t7z52u3k-fmvini-projects.vercel.app` (Ready em 30 s). Os aliases estáveis continuaram respondendo HTTP 200: `/health` na API e `/login` no frontend. O diretório Git local ficou limpo.

### Próximos passos

- O commit `34b0046` (`feat(deploy): publicar Vault com Vercel e Supabase`) e o commit de documentação `ad68e09` foram enviados para `origin/main`. Os projetos Vercel `vault-api` e `vault-web` foram conectados por `vercel git connect https://github.com/fmvini/Vault.git`; ambos mantêm seus Root Directories (`backend` e `frontend`). O push de `ad68e09` confirmou deploy automático em ambos os projetos.
